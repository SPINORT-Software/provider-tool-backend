...
class UserEntityDataAttributesModel:
    ...
    def get_role_attribute_groups(self, data_type_id, section_data):
        ...
        - Get only AttributeGroup objects which are not Conditional Display  
        """
        viewable_attribute_groups = AttributeGroup.objects.filter(attribute_set_id=role_attribute_set,
                                                                  parent_attribute_group__isnull=True,
                                                                  conditional_display=False)
        conditional_attribute_groups = AttributeGroup.objects.filter(attribute_set_id=role_attribute_set,
                                                                     parent_attribute_group__isnull=True,
    ...
    def list_attributes_data_type_by_role(self, role_id):
        """
        data_types are the sections under each user.
        Attribute groups are forms under sections.
        An attribute group can have sub group or a field directly.
